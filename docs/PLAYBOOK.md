...
**⚠️ 旧方式已废弃：** 直接 `claude --print` + 手动传 ENV（容易传错模型名）

**✅ 正确方式：CCS profile 调用（所有场景统一）**

```bash
# ✅ 标准委派（后台运行）
ccs minimax-ai claude --print --max-turns 30 << 'CC_INPUT'
[任务描述]
CC_INPUT

# ✅ 带超时包装（推荐，防止CC挂死）
timeout 600 bash -c 'ccs minimax-ai claude --print --max-turns 30' << 'CC_INPUT'
[任务描述]
CC_INPUT

# ✅ 后台运行（独立进程）
ccs minimax-ai claude --print --max-turns 30 << 'CC_INPUT' > /tmp/cc_output.txt 2>&1 &
echo "PID: $!"
CC_INPUT

# ✅ tmux 会话保持（会话需长期在线，只用于手动干预）
tmux kill-session -t cc-talentpilot 2>/dev/null; sleep 1
tmux new-session -d -s cc-talentpilot
tmux send-keys -t cc-talentpilot 'ccs minimax-ai claude' Enter
sleep 8
tmux send-keys -t cc-talentpilot '1' Enter   # workspace 信任
sleep 5
# 发送任务：
tmux send-keys -t cc-talentpilot 'task ...' Enter
```

**配置来源（不要手动复制）：**
- Profile: `~/.ccs/minimax-ai.settings.json`
- Base URL: `https://api.minimaxi.com/anthropic`（含 `/v1` 路径）
- Model: `MiniMax-M2.7`
- API Key: 已配置在 profile 中

| 参数 | 作用 |
| `ccs minimax-ai claude` | 使用 minimax-ai profile（已含所有 ENV），正确路由到 MiniMax API |
| `--print` | 结果输出到 stdout（非交互TTY），小P可捕获 |
| `--max-turns 30` | 防止无限循环，标准任务30步 |
| `--dangerously-skip-permissions` | 已内置在 profile 中，不需要手动加 |
