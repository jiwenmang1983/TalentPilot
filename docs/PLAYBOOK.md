...
### 3.1 调用方式（2026-05-10 更新）

**结论：CCS heredoc 是稳定方式，delegate_task 会被用户消息中断。**

#### 调用方式优先级

| 优先级 | 方式 | 稳定性 | 适用场景 |
|---|---|---|---|
| **P0（默认）** | `ccs minimax-ai claude --dangerously-skip-permissions --print --max-turns 100` (heredoc) | ✅ 独立进程，不阻塞 | **所有开发任务默认用这个** |
| P1 | `ccs` + `terminal(background=true)` | ✅ 独立进程+可追踪 | 长时间任务、需看中间输出 |
| P2 | `delegate_task(toolsets=["web"])` | ⚠️ 可被用户消息中断 | **仅用于研究/信息收集，不做代码修改** |

#### ✅ 标准委派命令（默认100步）

```bash
ccs minimax-ai claude --dangerously-skip-permissions --print --max-turns 100 << 'CC_INPUT'
...
# 监控是否在跑
ps aux | grep "claude --dangerously" | grep -v grep
...
```bash
# ❌ delegate_task(toolsets=["terminal","file"]) — 会被子Agent中断，不适合代码任务
# ❌ tmux send-keys — 只发文字到pane，CC不处理命令
# ❌ $(cat) 子 shell — ENV变量含特殊字符被截断，Not logged in
# ❌ 无 --dangerously-skip-permissions — 安全系统拦截，无法写文件
# ❌ 无 timeout 包装 — API无响应时会永远挂起
```
...
|---|---|
| 15步 | 1-2个文件，小改 |
| 25步 | 3-4个文件，或单一 Vue 页面 |
| 30步 | 5-6个文件，或前端页面 + router |
| 50步 | 7+文件（前后端全做、多文件前端） |
| **100步** | **默认**，复杂功能、完整模块 |
...
| 参数 | 作用 |
|---|---|
| `ccs minimax-ai claude` | 使用 minimax-ai profile，正确路由到 MiniMax API |
| `--dangerously-skip-permissions` | **必需**，否则安全系统拦截无法写文件 |
| `--print` | 结果输出到 stdout，小P可捕获 |
| `--max-turns 100` | **默认100步**，防止无限循环 |

**配置来源（不要手动复制）：**
- Profile: `~/.ccs/minimax-ai.settings.json`
- Base URL: `https://api.minimaxi.com/anthropic/v1`
- Model: `MiniMax-M2.7`（profile 内置）
...
```
1. 小P 写 prompt 到文件：/tmp/cc_task_<taskid>_en.txt（英文，详见3.2五段式）
2. 小P 用 terminal(background=True) 启动 CCS：
   ccs minimax-ai claude --dangerously-skip-permissions --print --max-turns 100 << 'CC_INPUT'
   [task description]
   CC_INPUT
3. CCS 在后台执行（独立进程，不阻塞 Hermes 主会话）
4. CCS 完成后，小P 检查 git log 验证结果
5. 小P 执行后续步骤：
   a. 构建验证（dotnet build / npm run build）
...
**Base URL：** `https://api.minimaxi.com/anthropic/v1`
**Model：** MiniMax-M2.7
**Key：** 已配置在 `~/.ccs/minimax-ai.settings.json`
**Rate Limit：** 429 → 等5分钟重试
